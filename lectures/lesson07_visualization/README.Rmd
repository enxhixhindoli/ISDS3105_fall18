...
ggplot(data = selfiesCasualties) +
  geom_bar(aes( x = class, y=..prop.., fill = gender, group = gender), position = 'dodge')

ggplot(data = freqCasualties) +
  geom_bar(aes( x = class,fill=gender)) #the dataset of frequencies there is one row for each level devide by gender

ggplot(data = freqCasualties) +
  geom_bar(aes( x = class, y = n, fill= gender), stat='identity')
...
```{r}
ggplot(data = selfiesCasualties) +
  geom_bar(aes( x = class, y=..count.., fill = gender, group = gender))
...
ggplot(data = dt) + geom_bar(aes(x=College, y=..count.., fill= Sex), position = 'fill')
...
#' 3.
ggplot(data = dt)+
 geom_bar(aes(x = College, y = ..prop.., group = 'hi'))
```
...
```{r}
ggplot(data = dt)+
 geom_bar(aes(x = College, y = ..prop.., fill =Sex, group = Sex), position = 'dodge')
...
ggplot(data = dt) + geom_bar(aes(x = Ethnicity, fill = Sex)) + facet_wrap(~College) + 
labs(title = "New plot title", subtitle = "A subtitle", caption = 'data from www.lsu.edu')
```
...
ggplot(data = dt) +
  ...
  scale_fill_manual(values  = c('turquoise3','lightsalmon' ))
#  OR (ALTERNATIVELY) change the colors usig :     scale_fill_brewer(type = ..., palette = ...)
dt %>% 
ggplot() + 
  geom_bar(aes(College, fill = Sex)) + 
  scale_fill_brewer(palette = "Oranges", name = 'Gender') +
...
#move the legend to the bottom. To find out how to do that, check the documentation for 
