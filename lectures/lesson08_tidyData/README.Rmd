---
title: 'Lesson 8: Tidy datasets for Dataviz"'
...
To plot in ggplot2 make sure you use [tidy datasets](http://r4ds.had.co.nz/tidy-data.html) (check
[this article](http://vita.had.co.nz/papers/tidy-data.html) for a more in depth explanation). Tidy datasets (also called _longitudinal_ or _normal_) comply with three rules:
...
What rules of the 3 mentioned above does `sn_users` violate? Could you still plot a line chart of the number of users by year?
...
```{r}
?geom_line
tidy_sn_user <- gather(sn_users, key = 'socialNet', value='Mil.Users', -year)
tidy_sn_user
ggplot(tidy_sn_user, aes(x=year)) +
  geom_line(data=tidy_sn_user)
```
...
Using the dataset `iphoneSales`, we want to plot the iphone sold worldwide (in millions) in each quarter. First, we need to tidy `iphoneSales`:
...
If you want to **untidy** datasets, for example to make them more readable, use `tidyr::spread()`. For instance, try to use `spread()` to untidy `socialNetwork_users`.
